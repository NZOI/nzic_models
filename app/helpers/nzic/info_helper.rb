module NZIC::InfoHelper
  ...
  def markdown_parse(str, root: root_path)
    content = NZIC::InfoHelper.markdown_renderer.render(str || "")
    content = Loofah.fragment(content)
      .scrub!(Loofah::Scrubbers::NZIC::NoComment.new)
      .scrub!(:escape) # remove dangerous tags
      .scrub!(Loofah::Scrubbers::NZIC::NoForm.new)
    unless root.nil?
      content = content.scrub!(Loofah::Scrubbers::NZIC::RootLink.new(root)) # re-write root-relative links as rooted links
    end
